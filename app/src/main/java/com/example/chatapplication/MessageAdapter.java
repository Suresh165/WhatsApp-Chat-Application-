...
public class MessageAdapter extends RecyclerView.Adapter <MessageAdapter.Messageholder>{
    ...
    public void onBindViewHolder(@NonNull Messageholder holder, int position) {
        ...
        if (messages.get(position).getSender().equals(FirebaseAuth.getInstance().getCurrentUser().getEmail())){
            Glide.with(context).load(senderImg).error(R.drawable.account).placeholder(R.drawable.account).into(holder.profileImage);
            ...
            Glide.with(context).load(receiverImg).error(R.drawable.account).placeholder(R.drawable.account).into(holder.profileImage);
            ConstraintSet constraintSet = new ConstraintSet();
            constraintSet.clone(constraintLayout);
